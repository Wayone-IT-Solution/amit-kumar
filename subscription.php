...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['subscription_id'])) {
    try {
        ...
        // Set success message and stay on same page
        $success_msg = "Subscription order #$order_code created successfully! Amount: ₹" . number_format($subscription['price'], 2) . ". Please complete payment to activate your subscription.";
...
<head>
  ...
  <style>
        .subscription-hero {
            background: 
            url('assets/img/about/about1.png');
            padding: 80px 0;
            position: relative;
            overflow: hidden;
        }
        ...
        <?php foreach ($subscriptions as $sub): 
          ...
          $subImage = (!empty($subImageFile) && file_exists(__DIR__ . '/' . $imagePath)) 
                    ...
                    <div class="col-lg-4 col-md-6 subscription-card-wrapper" data-aos="fade-up" data-aos-delay="<?= $loop * 100 ?>">
                        <div class="subscription-card d-flex flex-column h-100" style="min-height: 540px;">
                            <!-- Price Badge -->
            ...
            <div class="card-body text-center d-flex flex-column flex-grow-1">
                                ...
                                <!-- Subscribe Button -->
                                <button class="btn subscribe-btn mt-auto w-100" style="margin-top:auto;" onclick="openSubscriptionModal(<?= $sub['id'] ?>, '<?= htmlspecialchars($sub['title']) ?>', <?= $sub['price'] ?>, <?= $sub['valid_days'] ?>)">
...
<script src="assets/js/main.js"></script>
    ...
    <script>
                ...
                onDayCreate: function (_, __, ___, dayElem) {
                    ...
                    if (blockedDates.includes(dateStr)) {
                        ...
                        dayElem.addEventListener('click', function (e) {
                            e.preventDefault();
                            showError('Unavailable Date', 'This date is unavailable for delivery. Please choose another.');
                        });
                    }
                },
                onChange: function (selectedDates, dateStr) {
                    if (blockedDates.includes(dateStr)) {
                        showWarning('Blocked Date', 'This date is blocked. Please choose another.');
                        this.clear();
                    }
                }
            });
        ...
        <?php if (isset($success_msg)): ?>
            showSuccess('Success!', '<?= $success_msg ?>');
        <?php endif; ?>
