                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                        <h4 class="mb-sm-0">Deliverd Orders</h4>
                    ...
                    <div class="table-responsive table-card mt-3 mb-1">
                        <table class="table align-middle table-nowrap" id="orderTable">
                            ...
                            <tbody class="list form-check-all">
                                <?php
                                require '../inc/db.php';
                                $stmt = $conn->prepare("SELECT * FROM orders WHERE order_status = 'delivered' ORDER BY id DESC");
                                $stmt->execute();
                                ...
                                foreach ($orders as $order) {
                                    ...
                                    // Safely decode cart_data
                                    $cartDataRaw = $order['cart_data'] ?? '';
                                        ...
                                        foreach ($cartItems as $item) {
                                            $cartSummary .= '
                                                <div class="mb-2 border-bottom pb-1">
                                                    <strong>Product:</strong> ' . htmlspecialchars($item['product_name'] ?? '-') . ' (₹' . htmlspecialchars($item['product_price'] ?? '0') . ')<br>
                                                    <strong>Quantity:</strong> ' . htmlspecialchars($item['quantity'] ?? '-') . '<br>
                                                    <strong>Weight:</strong> ' . htmlspecialchars($item['product_weight'] ?? '-') . '<br>
                                                    <strong>Box:</strong> ' . htmlspecialchars($item['box_name'] ?? '-') . ' (₹' . htmlspecialchars($item['box_price'] ?? '0') . ')
                                                </div>';
                                        }
                                    } else {
                                        $cartSummary = '<div class="text-danger">Invalid or missing cart data.</div>';
                                    }
