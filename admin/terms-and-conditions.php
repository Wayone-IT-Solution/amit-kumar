...
$stmt = $conn->prepare("SELECT content FROM terms_and_conditions WHERE id = 1");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty(trim(strip_tags($content)))) {
            throw new Exception("Conditions content cannot be empty.");
        }

        // Check if disclaimer exists - update else insert
        $stmt = $conn->prepare("SELECT COUNT(*) FROM terms_and_conditions WHERE id = 1");
        $stmt->execute();
        $exists = $stmt->fetchColumn() > 0;

        if ($exists) {
            $update = $conn->prepare("UPDATE terms_and_conditions SET content = :content WHERE id = 1");
            $update->execute([':content' => $content]);
            $success = "Conditions updated successfully.";
        } else {
            $insert = $conn->prepare("INSERT INTO terms_and_conditions (id, content) VALUES (1, :content)");
            $insert->execute([':content' => $content]);
            $success = "Conditions added successfully.";
        }
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                <label for="descInput" class="form-label">Terms & Conditions</label>
                                <textarea name="content" id="content" rows="10" cols="80" class="form-control"
                                    placeholder="Enter Website terms and conditions"><?= htmlspecialchars($content) ?></textarea>
                            </div>
