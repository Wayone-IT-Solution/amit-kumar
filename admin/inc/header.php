...
// ✅ Fetch latest subscription order ID
$stmt = $conn->query("SELECT MAX(id) AS latest_id FROM subscription_orders");
$lastSubscriptionOrderId = $stmt->fetchColumn();
?>

<!-- ✅ Audio element for normal orders -->
<audio id="orderAlertSound" src="/amit-kumar/admin/assets/alert.mp3" preload="auto"></audio>
<!-- ✅ Audio element for subscription orders -->
<audio id="subscriptionAlertSound" src="/amit-kumar/admin/assets/alert.mp3.wav" preload="auto"></audio>
...
<div id="subscription-toast" style="display:none; position:fixed; top:70px; right:20px; background:#ffc107; color:#333; padding:12px 20px; border-radius:6px; z-index:9999;">
    <strong>New subscription order received!</strong>
    <a href="/amit-kumar/admin/subscription-list.php" style="color:#333; text-decoration:underline;">View</a>
</div>

<!-- ✅ SMS-Style Subscription Notification Popup -->
<div id="subscription-sms-popup" style="display:none; position:fixed; bottom:30px; right:30px; background:#fff; color:#333; border:2px solid #ffc107; box-shadow:0 4px 24px rgba(0,0,0,0.12); border-radius:16px; z-index:99999; min-width:320px; max-width:90vw; padding:18px 28px; font-size:1.1rem; font-family:'Segoe UI',Arial,sans-serif;">
    <div style="display:flex; align-items:center; gap:12px;">
        <div style="background:#ffc107; color:#fff; border-radius:50%; width:38px; height:38px; display:flex; align-items:center; justify-content:center; font-size:1.5rem;">
            <i class="bi bi-chat-dots"></i>
        </div>
        <div>
            <strong>New Subscription Order!</strong><br>
            <span id="subscription-sms-popup-msg">A new subscription order has been placed.</span>
            <div style="margin-top:6px;"><a href="/amit-kumar/admin/subscription-list.php" style="color:#ffc107; text-decoration:underline; font-weight:500;">View Details</a></div>
        </div>
    </div>
</div>
...
let lastSubscriptionOrderId = <?= $lastSubscriptionOrderId ?: 0 ?>;
...
function showSubscriptionSMSPopup(message) {
    const popup = document.getElementById('subscription-sms-popup');
    document.getElementById('subscription-sms-popup-msg').textContent = message || 'A new subscription order has been placed.';
    popup.style.display = 'block';
    setTimeout(() => {
        popup.style.display = 'none';
    }, 5000);
...
function checkNewSubscriptions() {
    fetch('/amit-kumar/admin/subscription-list.php?check_new=1')
        .then(res => res.json())
        .then(data => {
            if (!data || typeof data.latest_id === 'undefined' || isNaN(parseInt(data.latest_id))) return;
            const newId = parseInt(data.latest_id);
            if (newId > lastSubscriptionOrderId) {
                playSubscriptionSound();
                showSubscriptionPopup();
                showSubscriptionSMSPopup('A new subscription order has been placed.');
                // Voice notification
                if ('speechSynthesis' in window) {
                    const msg = new SpeechSynthesisUtterance('New subscription order received on Amit Dairy and Sweets.');
                    msg.lang = 'en-IN';
                    window.speechSynthesis.speak(msg);
                }
                lastSubscriptionOrderId = newId;
            }
        })
        .catch(err => {/* silent fail, do not break page */});
}
...
setInterval(checkNewSubscriptions, 2000);
</script>
