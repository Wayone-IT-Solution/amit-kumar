...
// Fetch latest subscription order ID for polling
$stmt = $conn->query("SELECT MAX(id) AS latest_id FROM subscription_orders");
$lastSubscriptionId = $stmt->fetchColumn();
?>

<!-- ✅ Audio element -->
<audio id="orderAlertSound" src="../assets/alert.mp3" preload="auto"></audio>
...
<!-- ✅ Audio element for subscriptions -->
<audio id="subscriptionAlertSound" src="../assets/alert.mp3" preload="auto"></audio>
<div id="subscription-toast" style="display:none; position:fixed; top:60px; right:20px; background:#ffc107; color:#333; padding:12px 20px; border-radius:6px; z-index:9999;">
    ...
    <a href="/amit-kumar/admin/subscription-list" style="color:#333; text-decoration:underline;">View</a>
</div>

<!-- ✅ JS Script to Check New Orders -->
<script>
let lastOrderId = <?= $lastOrderId ?: 0 ?>;
let lastSubscriptionId = <?= $lastSubscriptionId ?: 0 ?>;
...
function checkNewOrders() {
    fetch('/amit-kumar/admin/orders-list.php?check_new=1')
        ...
        .then(data => {
            ...
            // Notify for any new order, regardless of status
...
// Add polling for new subscriptions (voice only)
    ...
    fetch('/amit-kumar/admin/subscription-list.php?check_new=1')
        ...
        .then(data => {
            ...
            if (newId > lastSubscriptionId) {
                ...
                // Voice notification
                if ('speechSynthesis' in window) {
                    const msg = new SpeechSynthesisUtterance('New subscription received on Amit Dairy and Sweets.');
                    msg.lang = 'en-IN';
                    window.speechSynthesis.speak(msg);
                }
...
// Remove Test Voice and Force Notification debug buttons if present
const testBtn = document.getElementById('test-voice-btn');
if (testBtn) testBtn.remove();
const forceBtn = document.getElementById('force-notification-btn');
  ...
  <div class="collapse menu-dropdown" id="manageSubscriptionMenu">
    <ul class="nav nav-sm flex-column">
      <li class="nav-item">
        ...
        <li class="nav-item">
        <a href="newsletter" class="nav-link" data-key="t-team">Newsletter Subscription List</a>
      </li>
