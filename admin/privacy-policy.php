...
$stmt = $conn->prepare("SELECT content FROM privacy_policy WHERE id = 1");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM privacy_policy WHERE id = 1");
        $stmt->execute();
        $exists = $stmt->fetchColumn() > 0;

        if ($exists) {
            $update = $conn->prepare("UPDATE privacy_policy SET content = :content WHERE id = 1");
            $update->execute([':content' => $content]);
            $success = "Policy updated successfully.";
        } else {
            $insert = $conn->prepare("INSERT INTO privacy_policy (id, content) VALUES (1, :content)");
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                <label for="descInput" class="form-label">Privacy Policy</label>
                                <textarea name="content" id="content" rows="10" cols="80" class="form-control"
                                    placeholder="Enter Website Privacy Policy"><?= htmlspecialchars($content) ?></textarea>
                            </div>
