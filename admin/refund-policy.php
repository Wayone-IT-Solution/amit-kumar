...
$stmt = $conn->prepare("SELECT content FROM refund_policy WHERE id = 1");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM refund_policy WHERE id = 1");
        $stmt->execute();
        $exists = $stmt->fetchColumn() > 0;

        if ($exists) {
            $update = $conn->prepare("UPDATE refund_policy SET content = :content WHERE id = 1");
            $update->execute([':content' => $content]);
            $success = "Policy updated successfully.";
        } else {
            $insert = $conn->prepare("INSERT INTO refund_policy (id, content) VALUES (1, :content)");
    ...
    <div class="col-xxl-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">Refund Policy</h4>
            </div>
            <div class="card-body">
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                <label for="descInput" class="form-label">Refund Policy</label>
                                <textarea name="content" id="content" rows="10" cols="80" class="form-control"
                                    placeholder="Enter Website Refund Policy"><?= htmlspecialchars($content) ?></textarea>
                            </div>
