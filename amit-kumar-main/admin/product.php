...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO products 
            (product_code, category_id, subcategory_id, name, description, weight, weight_type, price, tags, discount_price, min_order, max_order, product_image)
            VALUES (:pid, :cat, :subcat, :name, :desc, :weight, :wtype, :price, :tags, :d_price, :min, :max, :img)");
        $stmt->execute([
            ':pid'      => $productCode,
            ':cat'      => $category_id,
...
function deleteProduct(id) {
    if (confirm("Are you sure you want to delete this Product?")) {
        window.location.href = 'inc/delete_product?id=' + id;
    }
}
</script>
